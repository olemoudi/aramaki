...
ASPNET = '|File\.|StreamReader|StreamWriter|TextReader|TextWriter|Directory\.|DirectoryInfo|ApplicationPhysicalPath|ApplicationVirtualPath|Path\.ChangeExtension|Path\.Combine|Path\.Get|Path\.HasExtension|Path\.IsRooted|\.ashx|\.axd|IHttpModule|HttpApplication|AcquireRequestState|AuthenticateRequest|AuthorizeRequest|BeginRequest|EndRequest|LogRequest|MapRequestHandler|PostAcquireRequestState|PostAuthenticateRequest|PostAuthorizeRequest|PostLogRequest|PostMapRequestHandler|PostReleaseRequestHandler|PostReleaseRequestState|PostRequestHandlerExecute|PostResolveRequestCache|PreRequestHandlerExecute|PreSendRequestContent|PreSendRequestHeaders|ReleaseRequestState|ResolveRequestCache|UpdateRequestCache|IHttpHandler|ProcessRequest|httpModules|httpHandlers|Global.asax|Application_Start|Application_End|Application_Error|Session\[|Request\[|\.Params|\.QueryString|\.ServerVariables|\.Headers|\.Url|\.RawURL|\.UrlReferrer|\.Cookies|\.BinaryRead|\.HTTPMethod|\.Browser|\.UserAgent|\.AcceptTypes|\.UserLanguages|FileStream|StreamReader|StreamWriter|SqlCommand|SqlDataAdapter|OleDbCommand|SqlCeCommand|Process|ProcessStartInfo|Redirect\(|Status\(|StatusCode\(|AddHeader\(|AppendHeader\(|Transfer\(|httpCookies|sessionstate|compilation|customErrors|httpRunTime'
PHP = "|\$_GET|\$_POST|\$_REQUEST|\$HTTP_.*_VARS|\$_COOKIE|\$_FILES|REQUEST_METHOD|QUERY_STRING|REQUEST_URI|SERVER\[.?HTTP_|PHP_SELF|fopen|readfile|file\(|fpassthru|gzopen|gzfile|gzpassthru|readgzfile|copy|rename|rmdir|mkdir|unlink|file_.*_contents|parse_ini_file|include_?|require_?|virtual|_query|eval\(|call_user|create_function|exec\(|passthru|popen|proc_open|shell_exec|system\(|http_redirect|header|httpmessage::setresponsecode|httpmessage::setheaders|socket_|fsockopen|->prepare|->bind_param|->execute|rand\(|mt_rand\(|_prepare|register_globals|safe_mode|mail\(|magic_quotes|allow_url_|display_errors|file_uploads|upload_tmp_dir|post_max_size|upload_max_filesize|preg_|ereg_"
...
CUSTOM = "|hack|fixme|todo|rush|security|insecure|hardcoded|hard-coded|hardwired|hard-wired|fixed key|fixed-key|[a-fA-F0-9]{16,}|[a-zA-Z0-9+/]{11}=|[a-zA-Z0-9+/]{22}==|[a-zA-Z0-9+/]{27}=|[a-zA-Z0-9+/]{43}=|regex|enkrypt|encrypt|crypt|clave|password|passwd|pwd|login|key|cipher|md5|sha1|hash|digest|sign|firma|b64|echo|https?:|://|\w+@\w+|email|aHR0|%2e%2e|random|certificate|cert|uuid|imei|imsi|sql|\d+\.\d+\.\d+\.\d+"
...
def printSlide(slide, win, footer, flagged=False):
    ...
    if len(slide) > MAXSOURCELINES:
        ...
        if len(slide[0][0]) + SHPAD + len(FILETEMP) > MAXCOLS:
            win.addstr('...'+str(slide[0][len(slide[0][0]) + SHPAD + len(FILETEMP) - MAXCOLS + 4:]), curses.A_BOLD)
        else:
